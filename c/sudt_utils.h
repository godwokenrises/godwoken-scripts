...
int _sudt_emit_log(gw_context_t *ctx, uint32_t sudt_id,
                   uint8_t from_script_hash[32], uint8_t to_script_hash[32],
                   uint128_t amount,
  ...
  uint8_t *data = NULL;
#else
  static const uint32_t data_size = 32 + 32 + 16;
  uint8_t data[32 + 32 + 16] = {0};
  memcpy(data, from_script_hash, 32);
  memcpy(data + 32, to_script_hash, 32);
  memcpy(data + 32 + 32, (uint8_t *)(&amount), 16);
#endif
...
int sudt_get_balance(gw_context_t *ctx, uint32_t sudt_id, uint8_t address[32],
                     uint128_t *balance) {
  int ret = gw_verify_sudt_account(ctx, sudt_id);
  if (ret != 0) {
    return ret;
  }
  return _sudt_get_balance(ctx, sudt_id, address, balance);
}

/* Transfer Simple UDT */
int _sudt_transfer(gw_context_t *ctx, uint32_t sudt_id, uint8_t from_script_hash[32],
                   uint8_t to_script_hash[32], uint128_t amount,
                   uint8_t service_flag) {
  int ret;
  if (memcmp(from_script_hash, to_script_hash, 32) == 0) {
  ...
  ret = _sudt_get_balance(ctx, sudt_id, from_script_hash, &from_balance);
  if (ret != 0) {
    return ret;
  }
  ...
  ret = _sudt_get_balance(ctx, sudt_id, to_script_hash, &to_balance);
  if (ret != 0) {
    return ret;
  }
  ...
  /* update balance */
  ret = _sudt_set_balance(ctx, sudt_id, from_script_hash, new_from_balance);
  if (ret != 0) {
    return ret;
  }
  ret = _sudt_set_balance(ctx, sudt_id, to_script_hash, new_to_balance);
  if (ret != 0) {
    return ret;
  }
  return _sudt_emit_log(ctx, sudt_id, from_script_hash, to_script_hash, amount, service_flag);
}

int sudt_transfer(gw_context_t *ctx, uint32_t sudt_id, uint8_t from_script_hash[32],
                  uint8_t to_script_hash[32], uint128_t amount) {
  return _sudt_transfer(ctx, sudt_id, from_script_hash, to_script_hash, amount, GW_LOG_SUDT_TRANSFER);
}

/* Pay fee */
int sudt_pay_fee(gw_context_t *ctx, uint32_t sudt_id, uint8_t from_script_hash[32], uint128_t amount) {
  ...
  return _sudt_transfer(ctx, sudt_id, from_script_hash, to_script_hash, amount, GW_LOG_SUDT_PAY_FEE);
}
