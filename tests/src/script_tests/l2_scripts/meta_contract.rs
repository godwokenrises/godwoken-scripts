...
use crate::testing_tool::chain::{test_rollup_config, RollupConfigExtend};
use gw_common::{
    builtins::{CKB_SUDT_ACCOUNT_ID, RESERVED_ACCOUNT_ID},
    state::State,
    H256,
};
use gw_generator::{syscalls::error_codes::GW_ERROR_DUPLICATED_SCRIPT_HASH, traits::StateExt};
use gw_types::U256;
use gw_types::{
    core::ScriptHashType,
    packed::{AllowedTypeHash, CreateAccount, Fee, MetaContractArgs, Script},
    prelude::*,
};
...
fn test_meta_contract() {
    ...
    let rollup_config = test_rollup_config()
        .push_allowed_eoa_type(AllowedTypeHash::from_unknown(dummy_eoa_type_hash));
    ...
    let run_result = exec_env
        .execute(a_id, RESERVED_ACCOUNT_ID, args.as_bytes(), &block_info)
        .expect("execute");
...
fn test_duplicated_script_hash() {
    ...
    let run_result = exec_env
        .unhandle_execute(a_id, RESERVED_ACCOUNT_ID, args.as_bytes(), &block_info)
        .expect("unhandle execute");
    assert_eq!(run_result.exit_code, GW_ERROR_DUPLICATED_SCRIPT_HASH);
}
