...
#[test]
fn test_block_timestamp_smaller_or_equal_than_previous_block_in_submit_block() {
    // calculate type id
    let capacity = 1000_00000000u64;
    let spend_cell = build_always_success_cell(capacity, None);
    let input_out_point = random_out_point();
    ...
    let rollup_cell_data = {
        ...
        builder.tip_block_timestamp(block_timestamp).build()
    };
    let witness = {
        let rollup_action = RollupAction::new_builder()
            .set(RollupActionUnion::RollupSubmitBlock(
...
#[test]
fn test_block_timestamp_bigger_than_rollup_input_since_in_submit_block() {
    // calculate type id
    let capacity = 1000_00000000u64;
    let spend_cell = build_always_success_cell(capacity, None);
    let input_out_point = random_out_point();
...
#[test]
fn test_wrong_global_state_tip_block_timestamp_in_submit_block() {
    // calculate type id
    let capacity = 1000_00000000u64;
    let spend_cell = build_always_success_cell(capacity, None);
    let input_out_point = random_out_point();
