...
const INVALID_STAKE_CELL_ERROR: i8 = 27;
...
#[test]
fn test_stake_cells_more_than_reverted_blocks() {
    let input_out_point = random_out_point();
    let type_id = calculate_state_validator_type_id(input_out_point.clone());
    let rollup_type_script = {
        Script::new_builder()
    ...
    let burn_rate: u8 = rollup_config.reward_burn_rate().into();
    let reward_capacity: u64 = stake_capacity * 2 * burn_rate as u64 / 100;
    let received_capacity: u64 = reward_capacity + challenge_capacity;
    let burned_capacity: u64 = stake_capacity * 2 - reward_capacity;
    let receive_cell = CellOutput::new_builder()
        .capacity(CKBPack::pack(&received_capacity))
        .lock(reward_receive_lock)
        .build();
    ...
    let expected_err =
        ScriptError::ValidationFailure(INVALID_STAKE_CELL_ERROR).input_type_script(0);
    assert_error_eq!(err, expected_err)
}

#[test]
fn test_skip_reward_capacity_check_if_has_input_reward_lock() {
    let input_out_point = random_out_point();
    let type_id = calculate_state_validator_type_id(input_out_point.clone());
    let rollup_type_script = {
        Script::new_builder()
    ...
    let mut ctx = CellContext::new(&rollup_config, param);
    let stake_capacity = 10000_00000000u64;
    let input_reward_lock_cell = {
        let cell = CellOutput::new_builder()
            .capacity(CKBPack::pack(&stake_capacity))
            .lock(reward_receive_lock.clone())
            .build();
        let out_point = ctx.insert_cell(cell, Bytes::new());
        CellInput::new_builder().previous_output(out_point).build()
    };
    ...
    let burn_rate: u8 = rollup_config.reward_burn_rate().into();
    let reward_capacity: u64 = stake_capacity * 2 * burn_rate as u64 / 100;
    let received_capacity: u64 = (reward_capacity + challenge_capacity) / 10; // Do contribution
    let burned_capacity: u64 = stake_capacity * 2 - reward_capacity;
    let receive_cell = CellOutput::new_builder()
        .capacity(CKBPack::pack(&received_capacity))
        .lock(reward_receive_lock)
